...
-c
	Compile only: create a .o file, don't create an executable (such as a.out)
-o filename
	Name the output filename instead of the default a.out
-g
	Put some extra information in the output files (.o and executables) that can be used by the debugger (debuggers are discussed in a future lab)
...
-pedantic
	 Issue all the warnings demanded by strict ISO C and ISO C++ (i.e. issue warnings if your program does not follow the standard exactly)
-std=c++11
	Use the newest C++ standard (C++11).  In the near future all C++ programs will need to conform to C++11.
